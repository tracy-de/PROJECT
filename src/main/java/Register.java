...
public class Register extends HttpServlet {
	...
	{		
		...
		//查詢買方是否重複信箱 無則新增
		String sql1="INSERT INTO client (client_mail,client_pwd) VALUES (?, ?)";
		String sql2="SELECT * FROM client WHERE client_mail=?";
		//查詢賣方是否重複信箱 無則新增
		String sql3="INSERT INTO company (company_mail,company_pwd) VALUES (?, ?)";
		...
		try {
			...
			if (rs.next()) {
				...
				out.print("<script>");
				out.print("setTimeout(function(){ window.location.href = 'http://localhost:8080/PROJECT/login.html'; }, 2000);");
				out.print("</script>");
				out.print("</body></html>");}
			else {
				...
				if (updatedata>0) {
					...
					out.print("<script>");
					out.print("setTimeout(function(){ window.location.href = 'login.html'; }, 2000);");
					out.print("</script>");
					out.print("</body></html>");
				}
			}
